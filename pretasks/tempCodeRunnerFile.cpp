...
class BlockList {
  private:
    ...
    #define MERGE_SIZE 290
    #define AMORT_BIAS 50
    ...
    // List header;  // Store list related inforatmion.

    /* Allocate one new list.
       Return the list index. */
    inline int newList() {
        int index = memory.nxt;
        index = 0; // JUST TEST USE
        if(index) { /* Not empty case. */
            read(memory.nxt,index); // Change memory.nxt
  ...
  public:
    ...
    BlockList(const char *s1,const char *s2):
        ...
        if(listFile.create()) {
            // system("pause");
            List tmp;// count = index = pre = nxt = 0
            tmp.index = -1;
            listFile.write(tmp);
            tmp.index = 0;
    ...
    void print() {
        ...
        while(index) {
            ...
            std::cout << "max:" << L.max.key << ' ' << L.max.val << '\n';
            index = L.nxt;
        }
        std::cout << "Counts:" << memory.count << '\n';
    }
    ...
    */
    ...
    void insert(const key_t &key,const value_t &val) {
        List tmp;               // Current List.
        read(tmp,0);        
        int index = tmp.nxt;    // current index.
        if(index == 0) { /* Special judge: empty list. */
            /* Write info about node 0. */
            index = tmp.pre = tmp.nxt = newList();
            write(tmp,0);
            /* Write info about new node. */
            Node cur;
            tmp.count = tmp.pre = tmp.nxt = 0;
            tmp.index = index;
            tmp.max = cur[0].get(key,val);
            write(tmp,index);
            write(cur,index);
            return;
        }
  ...
  protected:
    ...
    bool merge(List &tmp,Node &cur,bool type) {
        List tar; // Target List
        Node nxt; // Next-operated Node
